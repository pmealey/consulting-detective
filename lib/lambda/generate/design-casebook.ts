...
export const handler = async (state: CaseGenerationState): Promise<CaseGenerationState> => {
  ...
  const systemPrompt = `You are a game designer creating the casebook for a detective mystery game. The casebook is the player's address book — a list of places and people they can visit during their investigation. Each entry reveals specific facts and may be **gated** behind facts the player must discover first.
...
3. **Use person/place facts as gates**: Person-type facts gate person entries (e.g. discovering "fact_marsh" unlocks "Visit Harold Marsh"). Place-type facts gate location entries (e.g. discovering "fact_warehouse" unlocks "Warehouse at Limehouse").
...
- **Every fact must be reachable** through this chain: intro facts → first-wave entries → revealed facts → second-wave entries → more facts → ...
...
Example: If entry_lestrade reveals "fact_marsh_debt" (Marsh owed the victim £400), it must also reveal "fact_marsh" (the person identity fact for Harold Marsh). This way the player can follow up on the lead by visiting the now-unlocked "Harold Marsh" entry.
...
## Person Facts (identity atoms — use as gate keys for person entries)
